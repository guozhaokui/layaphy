...
export class VoxelLightRay {
    // 6个面的定义
    static FACE_FRONT: int = 0;
    static FACE_RIGHT: int = 1;
    static FACE_TOP: int = 2;
    static FACE_LEFT: int = 3;
    static FACE_BOTTOM: int = 4;
    static FACE_BACK: int = 5;

    faceLight: number[] = [0, 0, 0, 0, 0, 0];	// 当前光对6个面的贡献	
    maxLitFaceID = 0; 			// 主要对光面。 最亮的那个面
    dir = new Vector3();		// 规格化后的朝向
    path: number[] = [];			//光线路径，从000到终点，每个xyz记录的是与起点的偏移。不包含起点
     ...
     */
    static travelLine(x0: int, y0: int, z0: int, x1: int, y1: int, z1: int, visitor: Function/*(x:Number,y:Number,z:Number)=>boolean*/): void {
     ...
     * @param	ez
     */
    static creatLightLine(ex: int, ey: int, ez: int): VoxelLightRay {
        var ret = new VoxelLightRay();
        ret.setEnd(ex, ey, ez);
        return ret;
    }

    setEnd(ex: int, ey: int, ez: int): void {
        var ray = this.path;
        ray.length = 0;
        var first: Boolean = true;
        VoxelLightRay.travelLine(0, 0, 0, ex, ey, ez, function (x: int, y: int, z: int) {
            if (first) {
                first = false;
                return true;
            }
        ...
        var dirlen = Math.sqrt(ex * ex + ey * ey + ez * ez);
        var dirx = this.dir.x = ex / dirlen;
        var diry = this.dir.y = ey / dirlen;
        var dirz = this.dir.z = ez / dirlen;
        var fl = this.faceLight;
        ...
        this.maxLitFaceID = fl.reduce(function (maxLit: int, curv: Number, curid: int): int {
