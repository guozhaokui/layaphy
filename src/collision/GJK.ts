...
export class GJK {
	...
	Evaluate(shapearg: MinkowskiDiff, guess: Vec3) {
		...
		do {
			...
			switch (cs.rank) {
				case 2:
					sqdist = this.projectorigin2(cs.c[0].w, cs.c[1].w, weights, retMask);
					break;
				case 3:
					sqdist = this.projectorigin3(cs.c[0].w, cs.c[1].w, cs.c[2].w, weights, retMask);
					break;
				case 4:
					sqdist = this.projectorigin4(cs.c[0].w, cs.c[1].w, cs.c[2].w, cs.c[3].w, weights, retMask);
					break;
			}
	...
	EncloseOrigin(): boolean {
		...
		switch (m_simplex.rank) {
			case 1: {
				for (let i = 0; i < 3; ++i) {
					axis.set(i==0?1:0, i==1?1:0, i==2?1:0);
				...
				break;
			case 2: {
				m_simplex.c[1].w.vsub(m_simplex.c[0].w, d);//	let d = m_simplex.c[1].w - m_simplex.c[0].w;
				for (let i = 0; i < 3; ++i) {
					axis.set(i==0?1:0, i==1?1:0, i==2?1:0);
				...
				break;
			case 3: {
				m_simplex.c[1].w.vsub(m_simplex.c[0].w, d1);
				m_simplex.c[2].w.vsub(m_simplex.c[0].w,d2);
				...
				break;
			case 4: {
				m_simplex.c[0].w.vsub( m_simplex.c[3].w, d1);
				m_simplex.c[1].w.vsub( m_simplex.c[3].w, d2);
				m_simplex.c[2].w.vsub( m_simplex.c[3].w, d3);
	...
	appendvertice(simplex: GJK_sSimlex, v: Vec3) {
		...
		simplex.c[simplex.rank] = this.m_free[--this.m_nfree]; // 回收支撑点
